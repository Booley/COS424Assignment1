...
def tokenize_corpus(path, train=True):
    ...
    for dir in dirs:
        ...
        for f in files:
            ...
            tokens = list(nltk.bigrams(tokens)) # creates bigrams
...
def main(argv):
   ...
   if (not vocabf): 
      ...
      print "Now reading in vocabulary file"
      vocabfile = open(path+vocabf, 'r')
      vocab = [line.rstrip('\n') for line in vocabfile]
      newVocab = [] # tuple-ize list
      for i in range(0, len(vocab), 2):
        newVocab.append((vocab[i], vocab[i+1]))
      vocab = newVocab
      vocabfile.close()
   ...
   if (vocabf):
     ...
     vocab = [w for pair in vocab for w in pair]
     outfile.write("\n".join(vocab))
     outfile.close()
